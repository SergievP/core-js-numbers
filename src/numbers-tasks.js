...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(
    dotProductOfVectors / (vectorModule(x1, y1) * vectorModule(x2, y2))
  );
}
...
function isPrime(n) {
  if (n <= 1) return false;
  let maxSqrtNum = Math.floor(Math.sqrt(n));
  while (maxSqrtNum > 1) {
    if (n % maxSqrtNum === 0) return false;
    maxSqrtNum -= 1;
  }
  return true;
}
...
function getSumOfDigits(num) {
  ...
  return arrNum.reduce((sum, current) => sum + Number(current), 0);
}
...
function toPrecision(number, precision) {
  const IntLength = String(parseInt(number, 10)).length;
  const floatLength = precision - IntLength;
  return number.toFixed(floatLength);
}
